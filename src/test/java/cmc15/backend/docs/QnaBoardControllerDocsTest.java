    ...
    void QnA_게시판_페이지_조회_API() throws Exception {
        ...
        ResourceSnippetParameters resources = ResourceSnippetParameters.builder()
                ...
                .responseFields(
                        ...
                        fieldWithPath("data.content[].links[]").type(ARRAY).description("보험 추천 사이트 리스트"),
                        fieldWithPath("data.content[].links[].insuranceCompany").type(STRING).description("보험 추천된 회사 이름"),
                        fieldWithPath("data.content[].links[].link").type(STRING).description("보험 추천된 회사 사이트 주소"),
